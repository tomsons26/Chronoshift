...
MapEditClass::MapEditClass() : GameMouseClass()
{
    ...
    c.m_BasePercent = 100;
    Scen.Set_Waypoint(98, 0); // is it 98 here?
        ...
        switch (key) {
            ...
            case (KN_SHIFT_BIT | KN_HOME):
                if (CellClass::CurrentSelectedCell) {
                    if (Scen.Get_Waypoint(98) != -1) {
                        bool found = false;
                        for (int i = 0; i < 101; ++i) {
                            if (i != 98 && Scen.Get_Waypoint(98) == Scen.Get_Waypoint(i)) {
                                found = true;
                            }
                        }
                        if (!found) {
                            Array[Scen.Get_Waypoint(98)].Set_Bit16(false);
                            Flag_Cell(Scen.Get_Waypoint(99));
                        }
                    }
                    Scen.Set_Waypoint(98, CellClass::CurrentSelectedCell);
            ...
            case (KN_SHIFT_BIT | KN_R):
                if (CellClass::CurrentSelectedCell && CellClass::CurrentSelectedCell != Scen.Get_Waypoint(98)) {
                    if (Scen.Get_Waypoint(99) != -1) {
                        bool found = false;
                        for (int i = 0; i < 101; ++i) {
                            if (i != 99 && Scen.Get_Waypoint(99) == Scen.Get_Waypoint(i)) {
                                found = true;
                            }
                        }
                        if (!found) {
                            Array[Scen.Get_Waypoint(99)].Set_Bit16(false);
                            Flag_Cell(Scen.Get_Waypoint(99));
                        }
                    }
                    Scen.Set_Waypoint(99, CellClass::CurrentSelectedCell);
                    Array[CellClass::CurrentSelectedCell].Set_Bit16(true);
                    Flag_Cell(CellClass::CurrentSelectedCell);
                    c.m_UnsavedChanges = true;
                    key = KN_NONE;
            ...
            case (KN_ALT_BIT | KN_SPACE):
                if (CellClass::CurrentSelectedCell) {
                    ...
                    */
                    if (CellClass::CurrentSelectedCell != Scen.Get_Waypoint(98)
                        && CellClass::CurrentSelectedCell != Scen.Get_Waypoint(99)) {
...
void MapEditClass::Draw_It(BOOL force_redraw)
{
    ...
    if (g_InMapEditor) {
        ...
        if (GameScreenClass::Buttons != nullptr && force_redraw && CurrentObjects.Count() > 0) {
            ...
            Fancy_Text_Print(namebuf, 160, 0, &ColorRemaps[PLAYER_COLOR_BROWN], 0, TPF_CENTER | TPF_NOSHADOW | TPF_EDITOR);
        }
    }
}
...
BOOL MapEditClass::Load_Scenario()
{
    ...
    /*
    if (player == 5) {
        Clear_Vector(&dword_699160, v5, v4 = *(&a1 + 1) >> 0x18);
        v6 = operator new(0x1Du);
        if (v6) {
            v6 = (IPXAddressClass::IPXAddressClass((v6 + 0xC)) - 0xC);
        }
        v7 = &unk_69858A;
        v8 = v6;
        v15 = v6;
        ...
        v15[0x16] = byte_698598;
        v15[0x17] = BYTE3(dword_698594);
        DynamicVectorClass<NodeNameTag *>::Add(&dword_699160, &v15);
        dword_6985A5 = 1;
        c.m_CurrentOwner = 12;
    } else */
    {
...
int MapEditClass::Placement_Dialog()
{
    ...
    while (process) {
        ...
        if (to_draw) {
            ...
            WindowList[5].H = 0x69;
            Change_Window(5);
            // Draw_Box(0x23, v18, 0x98, v19, 0, 0);
            // a lot of code
            // Fancy_Text_Print(v51, 0x6F, 0x20, v50, 0, 0x119);
            // a lot of code
            activegdt->Draw_All(true);
            g_mouse->Show_Mouse();
            to_draw = false;
...
void MapEditClass::Update_Waypoint(int wp)
{
    ...
    if (waypoint != -1) {
        // TODO is it checking 98 and 99?
        if (waypoint != Scen.Get_Waypoint(98) && waypoint != Scen.Get_Waypoint(99)) {
