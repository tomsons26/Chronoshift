...
private:
    ...
    int Save_Scenario();
    int Scenario_Dialog();
    int Pick_Scenario(char *caption, int &scen_num, ScenarioPlayerEnum &player, ScenarioDirEnum &dir, ScenarioVarEnum &var);
    int Size_Map(int x, int y, int width, int height);
    void Popup_Controls();
    void AI_Menu();
    void Handle_Teams(char *caption);
    ...
    void Draw_Member(TechnoTypeClass *techtype, int index, int count, HousesType house);
    BOOL Get_Waypoint_Name(char *waypoint_name);
    void Set_House_Buttons(HousesType house, GadgetClass *gadget, int index);
...
protected:
    ...
    struct Struct
    {
        ...
        int m_LeftClickTime; // time the left button was last clicked
