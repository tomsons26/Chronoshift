            ...
            if (Map.In_Radar(calc_cell)) {
                ...
                if (type->Is_Bib()) {
                    ...
                    if (type->Is_Crater() && cptr->Has_Crater()) {
                        cptr->Set_Smudge_Frame(cptr->Get_Smudge_Frame() + 1);
                        cptr->Set_Smudge_Frame(std::min(cptr->Get_Smudge_Frame(), (uint8_t)4));
                    }
                    if (cptr->Get_Smudge() == SMUDGE_NONE) {
                        cptr->Set_Smudge(type->Is_Crater() ? (SmudgeType)cptr->Spot_Index(m_Coord) : type->What_Type());
    ...
    for (int i = 0; i < MAP_MAX_AREA; ++i) {
        ...
        if (cptr->Get_Smudge() != SMUDGE_NONE) {
            ...
            if (stptr != nullptr && !stptr->Is_Bib()) {
                ...
                std::snprintf(entry, sizeof(entry), "%s,%d,%d", stptr->Get_Name(), i, cptr->Get_Smudge_Frame());
